...
function CompanyDashboardMenuSide({ onNavigate, currentTab, onBack }) {
  const { user, signOut } = useAuth()

  const menuItems = [
    { id: 'dashboard', label: 'Dashboard', icon: 'dashboard' },
    { id: 'students', label: 'Colaboradores', icon: 'group' },
    ...
    { id: 'compliance', label: 'Conformidade', icon: 'security' },
    { id: 'reports', label: 'Relatórios', icon: 'assessment' },
  ...
  return (
    <aside className="fixed inset-y-0 left-0 z-[60] flex flex-col h-screen w-64 bg-white border-r border-slate-200 font-headline text-sm font-medium tracking-tight">
      ...
      <div className="p-6 flex items-center gap-3">
        ...
        <div>
          <h1 className="text-sm font-bold text-slate-900 leading-tight">NR-01 Company</h1>
          <p className="text-[10px] text-slate-500 uppercase tracking-wider">Portal do Colaborador</p>
        </div>
      </div>
      ...
      <div className="p-4 border-t border-slate-200 space-y-3">
        ...
        <div className="flex items-center gap-3 px-2 py-2">
          <div className="w-10 h-10 rounded-full bg-blue-100 flex items-center justify-center">
            <span className="material-symbols-outlined text-slate-700 text-sm">business</span>
          </div>
          <div className="overflow-hidden flex-1">
            <p className="text-xs font-bold truncate text-slate-900">{user?.full_name || 'Company'}</p>
            <p className="text-[10px] text-slate-500 truncate">{user?.email || 'company@nr01.com.br'}</p>
          </div>
          <button
...
export default CompanyDashboardMenuSide
